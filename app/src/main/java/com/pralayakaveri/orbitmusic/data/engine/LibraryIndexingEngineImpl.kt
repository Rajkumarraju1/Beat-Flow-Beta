...
import com.pralayakaveri.orbitmusic.data.observer.MediaStoreObserver
import com.pralayakaveri.orbitmusic.data.provider.MediaStoreProvider
import com.pralayakaveri.orbitmusic.domain.engine.*
...
class LibraryIndexingEngineImpl @Inject constructor(
    ...
    init {
        ...
        // REMOVED: Initial Scan from init. This must be triggered manually by MainActivity.
    }
    ...
    override suspend fun startSync(reason: TriggerReason, forceFullScan: Boolean) = withContext(Dispatchers.IO) {
        ...
        // ATOMIC GUARD: Reject concurrent syncs immediately
        if (!isSyncing.compareAndSet(false, true)) {
            android.util.Log.d("LibraryIndexingEngine", "[SYNC_LOCK] Scan already running (Trigger: $reason), skipping.")
            if (reason != TriggerReason.FOLLOW_UP_COALESCED) {
                isFollowUpPending = true
            }
            return@withContext
        }
            ...
            try {
                ...
                if (idsToDeepScan.isNotEmpty()) {
                    ...
                    if (deepSongs.isNotEmpty()) {
                        ...
                        android.util.Log.w("INDEX_DEBUG", "Deep scan returned 0 songs for ${idsToDeepScan.size} IDs.")
                    }
                    ...
                    android.util.Log.d("INDEX_DEBUG", "Successfully processed deep scan batch. Added: $added")
                }
                ...
                if (mediaStoreHeadless.isNotEmpty() && forceFullScan && added == 0) {
                    val errorMsg = "INDEX_CRITICAL: Force rescan found ${mediaStoreHeadless.size} songs but ADDED 0 to DB. Aborting transaction to prevent data loss."
                    android.util.Log.e("LibraryIndexingEngine", errorMsg)
                    throw IllegalStateException(errorMsg)
                }
                
                if (mediaStoreHeadless.isNotEmpty() && added == 0 && idsToDeepScan.isNotEmpty()) {
                    android.util.Log.e("INDEX_CRITICAL", "Insert pipeline failure detected: processed=${mediaStoreHeadless.size} added=0")
                }
            }
        } catch (e: Exception) {
