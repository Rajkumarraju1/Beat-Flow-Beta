...
import com.pralayakaveri.orbitmusic.data.local.LibraryPreferencesManager
...
import com.pralayakaveri.orbitmusic.domain.util.LyricsParser
...
class MainViewModel @Inject constructor(
    ...
    private val themeManager: com.pralayakaveri.orbitmusic.presentation.theme.ThemeManager,
    private val syncManager: com.pralayakaveri.orbitmusic.domain.util.LyricsSyncManager,
    ...
    private val visualizerHelper = com.pralayakaveri.orbitmusic.domain.util.AudioVisualizerHelper()
    ...
    val currentTheme = themeManager.themeFlow.stateIn(
        viewModelScope, SharingStarted.WhileSubscribed(5000), com.pralayakaveri.orbitmusic.presentation.theme.ThemeType.DYNAMIC
    )
    ...
    val playlists: StateFlow<List<com.pralayakaveri.orbitmusic.domain.model.Playlist>> = musicRepository.getPlaylists()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    init {
        ...
        viewModelScope.launch {
            indexingState.collect { state ->
                ...
                } else if (state == com.pralayakaveri.orbitmusic.domain.engine.IndexingState.IDLE) {
                    retryCount = 0
                }
            }
        }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val parsedLyrics = if (localContent != null) {
                    if (com.pralayakaveri.orbitmusic.domain.util.LrcParser.isLrcFormat(localContent)) {
                        com.pralayakaveri.orbitmusic.domain.util.LrcParser.parseLrcContent(localContent)
                    } else {
                        com.pralayakaveri.orbitmusic.domain.util.LrcParser.parsePlainText(localContent, song.duration)
                    }
                } else {
    ...
    fun setTheme(themeType: com.pralayakaveri.orbitmusic.presentation.theme.ThemeType) = viewModelScope.launch { themeManager.setTheme(themeType) }
    fun toggleFavorite(targetSongId: Long) = viewModelScope.launch { musicRepository.toggleFavorite(targetSongId) }
    fun onSearchQueryChange(query: String) { _searchQuery.value = query }
    private var retryCount = 0

    fun forceRescan() {
        if (indexingState.value != com.pralayakaveri.orbitmusic.domain.engine.IndexingState.IDLE) {
